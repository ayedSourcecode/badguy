...
class WeekDataC {
	public static var weeksLoaded:Map<String, WeekDataC> = new Map<String, WeekDataC>();
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				...
				if(!weeksLoaded.exists(sexList[i])) {
					...
					if(week != null) {
						var weekFile:WeekDataC = new WeekDataC(week, sexList[i]);
		...
		for (i in 0...directories.length) {
			var directory:String = directories[i] + 'covers/';
	...
	private static function addWeek(weekToCheck:String, path:String, directory:String, i:Int, originalLength:Int)
	{
		if(!weeksLoaded.exists(weekToCheck))
		{
			...
			if(week != null)
			{
				var weekFile:WeekDataC = new WeekDataC(week, weekToCheck);
				if(i >= originalLength)
				{
					#if MODS_ALLOWED
	...
	public static function getCurrentWeek():WeekDataC {
		return weeksLoaded.get(weeksList[PlayState.storyWeek]);
	}

	public static function setDirectoryFromWeek(?data:WeekDataC = null) {
